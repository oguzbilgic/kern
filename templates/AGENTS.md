...
**Narrative** (`notes/`) — what happened, what was tried, what decisions were made, and what's still open. One file per day, named `YYYY-MM-DD.md`. Append-only. Never modify a past day's entry. Lets a new session pick up exactly where the last one left off.
