...
let _getRecallStats: (() => { chunks: number; sessions: number } | null) | null = null;
...
export function setRecallStatsFn(fn: () => { chunks: number; sessions: number } | null) {
  _getRecallStats = fn;
}
...
export function getStatusData(): StatusData {
  ...
  return {
    ...
    recall: _getRecallStats ? (() => {
      ...
      return rs ? `${rs.chunks} chunks indexed` : "disabled";
    })() : null,
  };
}
