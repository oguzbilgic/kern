...
| `GIT_REMOTE_URL` | Git remote URL for the agent repo. Set in Docker to auto-configure `origin` on first run. Use a token URL for auth (e.g. `https://x-access-token:ghp_xxx@github.com/org/repo.git`). See [deployment.md](deployment.md#git-sync). |
