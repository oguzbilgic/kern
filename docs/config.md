...
| `summaryModel` | `""` | Model for segment summarization. Empty = provider default (`gpt-4.1-mini` on OpenAI, `claude-haiku-4.5` on Anthropic via OpenRouter, `openai/gpt-4.1-mini` on OpenRouter, reuses `model` on Ollama). Routed through the agent's provider. Useful when the main `model` is a thinking model — thinking burns the summary token budget on reasoning and returns empty text. Set this to a non-thinking model (e.g. `"qwen3:4b-instruct"`, `"gemma4:e2b"`, `"openai/gpt-4.1-mini"`). |
