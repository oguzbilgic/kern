...
- **Hardened default `.gitignore`** ([#226](https://github.com/oguzbilgic/kern-ai/issues/226)) — added `.kern/agent.pid` and swapped `.kern/recall.db` for the catch-all `.kern/*.db`. Only affects new agents from `kern init`
- **Init templates** ([#235](https://github.com/oguzbilgic/kern-ai/pull/235)) — moved `IDENTITY.md`, `KNOWLEDGE.md`, and `USERS.md` from inline strings into `templates/` alongside `AGENTS.md`, so they can be edited without rebuilding. `IDENTITY.md` is now a warm bootstrap prompt asking the agent to have a first conversation with its human and fill in name / purpose / vibe, instead of hardcoding the capitalized directory name. `USERS.md` and `KNOWLEDGE.md` are terse empty-state templates that nudge the agent to fill them in as they go
