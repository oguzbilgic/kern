...
- **Hardened default `.gitignore`** ([#226](https://github.com/oguzbilgic/kern-ai/issues/226)) — added `.kern/agent.pid`, `skills-lock.json`, and swapped `.kern/recall.db` for the catch-all `.kern/*.db`. Only affects new agents from `kern init`
- **Init templates** — moved `IDENTITY.md`, `KNOWLEDGE.md`, and `USERS.md` from inline strings into `templates/` alongside `AGENTS.md`. Identity template now prompts the agent to ask its human on first conversation instead of hardcoding the capitalized name
