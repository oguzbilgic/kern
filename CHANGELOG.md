...
### Features
- **Background execution** — `bash` tool now supports background execution and process management via the exec plugin. Fast commands return output directly (backward compatible). Slow commands auto-background after `yieldMs` (default 10s) and return a job with `logFile`. Use `background: true` to immediately run in background. PID reaper detects completion and injects a system message with log tail. `bash kill <pid>` to terminate. Log files at `.kern/jobs/`
